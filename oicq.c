...
prpl_login(PurpleAccount *acct)
{
	...
	sock_conn = axon_connect(purple_account_get_string(acct,
	    PRPL_ACCT_OPT_HOST, "localhost"),
...
static char *
prpl_get_cb_real_name(PurpleConnection *gc, int id, const char *who)
{
	return NULL;
}
...
{
	PurpleConversation *conv;
	const char *s_id = g_hash_table_lookup(components,
	    PRPL_CHAT_INFOID);
	const char *s_name = g_hash_table_lookup(components,
	    PRPL_CHAT_INFONAME);
...
prpl_init(PurplePlugin *plugin)
{
	...
	protocol_options = g_list_append(protocol_options,
	    purple_account_option_list_new("登录方式", PRPL_ACCT_OPT_LOGIN,
		    login_options));
	protocol_options = g_list_append(protocol_options,
	    purple_account_option_list_new("协议类型", PRPL_ACCT_OPT_PROTO,
		    login_proto_opts));
	protocol_options = g_list_append(protocol_options,
	    purple_account_option_string_new(
	    "服务器主机名", PRPL_ACCT_OPT_HOST,
	    "127.0.0.1"));
	protocol_options = g_list_append(protocol_options,
	    purple_account_option_string_new(
	    "服务器端口号", PRPL_ACCT_OPT_PORT,
	    "9999"));
